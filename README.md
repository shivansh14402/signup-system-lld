   ...
   Strategy       Strategy        Strategy
      ...
      └───────────────┼────────────────┘
          ...
          Authentication Service
                 ...
                 Auth Token
🔄 Signup Flow
Client sends a signup request with the selected provider.
SignupService receives the request.
Factory returns the appropriate verification strategy.
Strategy validates/verifies the user.
Repository persists the verified user.
Authentication service generates an auth token.
...
Create VerifyUserStrategyApple
Register it inside VerifyUserStrategyFactory
Add APPLE to the signup method enum
